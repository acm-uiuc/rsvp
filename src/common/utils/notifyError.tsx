...
function ErrorNotificationMessage({ error }: { error: ApiError }) {
  ...
  return (
    <Stack gap={4}>
      ...
      {error.requestId && (
        <>
          <Anchor size="xs" onClick={() => setShowDetails(d => !d)} style={{ cursor: 'pointer' }}>
            {showDetails ? 'Hide details' : 'View details'}
          </Anchor>
          {showDetails && (
            <Group gap="xs" align="center" wrap="nowrap">
              <Text size="xs" c="dimmed" style={{ whiteSpace: 'nowrap' }}>Request ID:</Text>
              <Code style={{ wordBreak: 'break-all', fontSize: '0.7rem', flex: 1 }}>{error.requestId}</Code>
            </Group>
          )}
        </>
      )}
...
export function showApiErrorNotification(err: ApiError | unknown): void {
  ...
  notifications.show({
    ...
    autoClose: false,
    message: <ErrorNotificationMessage error={error} />,
  });
}
